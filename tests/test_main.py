...
db_save_patcher = patch(
    "learning_agent.main.save_bias_state",
    side_effect=mock_save_bias_state,
)
db_load_patcher = patch(
    "learning_agent.main.load_bias_state",
    side_effect=mock_load_bias_state,
)
db_init_patcher = patch(
    "learning_agent.main.init_db",
    return_value=None,
)
...
class TestMain(unittest.TestCase):
    def setUp(self):
        """Set up the client and reset mock database state for each test."""
        global mock_db_state
        mock_db_state.clear()
    ...
    def test_health_check(self):
        with patch(
            "learning_agent.main.check_db_connection",
            return_value=True,
        ):
            ...
            self.assertEqual(
                data["data"]["outcome_contract_version"],
                "learning-outcome-v1",
            )
            self.assertIn("timestamp", data)

    def test_update_biases_single(self):
        request_body = self._get_base_bias_update_request(
            "AAPL",
            {"bull_bias": 0.1},
        )
        ...
        data = response.json()
        self.assertEqual(
            data["data"][0]["current_bias"]["bull_bias"],
            0.1,
        )
    ...
    @patch(
        "learning_agent.logic.fetch_trade_history",
    ...
    def test_learn_endpoint_with_mocks(self, mock_fetch_history):
        ...
        trades = [
            Trade(
                ...
                quantity=Decimal("1"),
                executed_at="2026-01-08T09:00:00Z",
    ...
    def test_bias_integration_in_learn_endpoint(
        self,
        mock_fetch_history,
    ):
        update_request = self._get_base_bias_update_request(
            "BTC-USD",
            {"bull_bias": 0.5},
        )
        response = self.client.post(
        ...
        self.assertIn(
            "BTC-USD",
            data["data"]["policy_deltas"]["asset_biases"],
        )
        self.assertGreater(
            data["data"]["policy_deltas"]["asset_biases"]["BTC-USD"],
            0,
        )
